...
export class Field extends Base {
  ...
  static parse(tokeniser) {
    ...
    const autoParenting = autoParenter(ret);
    autoParenting.extAttrs = ExtendedAttributes.parse(tokeniser);
    tokens.required = tokeniser.consume("required");
    autoParenting.idlType = type_with_extended_attributes(tokeniser, "dictionary-type") || tokeniser.error("Dictionary member lacks a type");
    tokens.name = tokeniser.consume("identifier") || tokeniser.error("Dictionary member lacks a name");
    autoParenting.default = Default.parse(tokeniser);
